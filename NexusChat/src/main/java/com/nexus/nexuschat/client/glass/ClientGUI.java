...
public class ClientGUI extends JFrame implements MessageListener {
    ...
    public ClientGUI(String username, ApplicationContext context) throws ExecutionException, InterruptedException {
        ...
        myStompClient = new MyStompClient(this, identityService.readIdentity().getUserId(), groupChatService, identityService);
    ...
    private void addMessageFeed() {
        ...
        PanelTransparent messageFeedPanel = new PanelTransparent(Utilities.TRANSPARENT_COLOR, false);
        ...
        inputField.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) {
                if((e.getKeyChar() == KeyEvent.VK_ENTER) && (!inputField.getText().isEmpty())){
                    ...
                    myStompClient.sendMessage(new Message(username, input, identityService.readIdentity().getUserId(), currentChatId, currentTime));
                }
            }
        });
    ...
    private JPanel createChatMessage(Message message, String userId){
        ...
        BubblePanel bubble = new BubblePanel(
                isSender ? new Color(0, 132, 255, 100) : new Color(230, 230, 230, 100));
    ...
    private void addGroupRequests() {
        ...
        createChatBtn.addActionListener(e -> {
            ...
            // Add local user first, stat = ACCEPTED
            selectedMembers.add(new GroupMember(
                    chatId, // chatId will be assigned later
                    localUserId,
                    localUsername,
                    "ACCEPTED"
            ));
            ...
            initialMembers.add(contactService.retrieveContactById(identityService.readIdentity().getUserId())); // add yourself
            for(GroupMember gm : selectedMembers){
                Contact member = contactService.createContact(gm.getReceiverId(), gm.getUsername());
            ...
            for(Contact c : contactService.retrieveAllContacts()){
            ...
            // Connect to this new group chat
    ...
    private JPanel createGroupRequestEntry(GroupRequest f) {
        ...
        acceptButton.addActionListener(e -> {
            ...
            // Add ALL members locally
            for (GroupMember g : allMembers) {
                Contact member = contactService.createContact(
                        g.getReceiverId(),
                        g.getUsername()
                );
                groupMembersToSave.add(member);

                // Only send friend requests for members other than yourself
                if (!g.getReceiverId().equals(identityService.readIdentity().getUserId())) {
                    myStompClient.sendFriendRequest(new FriendRequest(
                            UUID.randomUUID().toString(),
                            g.getReceiverId(),
                            identityService.readIdentity().getUserId(),
                            contactService.retrieveContactById(identityService.readIdentity().getUserId()).getUsername(),
                            g.getStat().equals("ACCEPTED") ? "ACCEPTED" : "PENDING",
            ...
            GroupChat joinedGroup = groupChatService.createGroupChat(
                    ...
                    f.getSenderId(),
                    f.getChatName(),
                    f.getRequestId() // <--- This is it, my destiny
            );
    ...
    public void onFriendRequestReceive(FriendRequest friendRequest) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if(friendRequest.getStat().equals("ACCEPTED")) {
                ...
                String contactName = friendRequest.getSenderId().equals(identityService.readIdentity().getUserId())
